...
create table if not exists transactions (
  ...
  authorization_status text not null default 'unknown' check (authorization_status in ('pending', 'posted', 'unknown')),
  pending_status text not null default 'none' check (pending_status in ('none', 'matched', 'confirmed_new', 'ignored_duplicate')),
  pending_match_transaction_id uuid references transactions(id) on delete set null,
  source_account_name text not null,
  source_account_type text not null check (source_account_type in ('transaction', 'credit_card', 'savings', 'loan')),
